...
for path, dirs, files in os.walk(dataDir):
    if path.split("/")[-1] != '.ipynb_checkpoints': 
        ...
        print(path)
    if path.split("/")[-1] in ['features2_small', 'features2_tiny']:
        for f in files:
            print('-'*8, f)
...
t1 = time.time()
[img_list, feats] = cPickle.load(open(os.path.join(dataDir, 'features2_tiny', '{}.p'.format(dataType)),'rb'),encoding='latin1')
...
for i in np.arange(len(img_list)):
    ...
    cat_index = np.concatenate((np.arange(2, 10), np.arange(16, 26)))

    cat_one_hot = np.zeros([18])

    for x in categories.intersection(cat_index):
        cat_one_hot[np.where(cat_index == x)] = 1.

    img_cat.append(cat_one_hot.tolist())

    print(cat_one_hot) # expect singleton

pd.DataFrame(img_cat).to_csv(dataDir + "/features2_small/img_cat_" + dataType + ".csv")
pd.DataFrame(feats.reshape(feats.shape[0], -1)).to_csv(dataDir + "/features2_small/feats_" + dataType + ".csv")
