...
class load_dataset:
    def __init__(self, data_dir):
        ...
        self.labelForBboxAllImageTrain = pd.read_csv(os.path.join(dataDir, 'bboxes', 'labelForBboxAllImage_train2014.csv'))
        
        print("load labels for bboxes successfully...")
        
        self.labelForBboxAllImageTrain = np.array(self.labelForBboxAllImageTrain.iloc[:, 1::])
        ...
        [self.img_idsTrain, self.featsTrain] = cPickle.load(open(os.path.join(dataDir, 'features2_small', 'train2014.p'),'rb'), encoding='latin1')
        ...
        # load features
        print("loading features...")
        [self.img_idsCv, self.featsCv] = cPickle.load(open(os.path.join(dataDir, 'features2_small', 'val2014.p'),'rb'), encoding='latin1')
        print("load features successfully...")
...
class LinearSVM:
    def __init__(self, regular_parameter, step_size, batch_size, initial_weight, train_size):
        ...
        self.maxiternum = np.floor(5 * train_size / batch_size)
    ...
    def objectiveLinearFunc(self, w, lambd, batch_x, batch_y):
        ...
        return lambd / 2. * torch.norm(w) ** 2. + 1. / (2. * len(batch_y)) * torch.sum(torch.clamp(1. - batch_y * batch_x.mm(w), min=0) ** 2.)
    ...
    def lossFunc(self, w, lambd, batch_x, batch_y):
        ...
        wn = w.data.numpy()
        loss = lambd.data.numpy()[0] / 2. * torch.norm(w).data.numpy() ** 2. + 1. / (2. * len(batch_y)) * (np.clip(1. - batch_y * batch_x.dot(wn), 0., None) ** 2.).sum()
        ...
        while n_iter < max_iter_num:
            ...
            for minibatch_y, minibatch_x in batch_iter_instant:
                ...
                if n_iter % np.floor(y.shape[0]/(2. * self.batchSize)) == 0:
                    ...
                    scores_train = x.dot(self.w.data.numpy())
                    labels_train = y
                    
                    pred_train = []
                    actual_train = []
                    
                    pred_train += [ s for (s, l) in zip(scores_train, labels_train) if s > 0 or l == 1]
                    actual_train += [ l for (s, l) in zip(scores_train, labels_train) if s > 0 or l == 1]
                    
                    self.trainAPScore.append(average_precision_score(actual_train, pred_train))

                    cv_loss = self.lossFunc(self.w, self.lambd, cvx, cvy)
                    self.cvLoss.append(cv_loss)
                    
                    scores_cv = cvx.dot(self.w.data.numpy())
                    labels_cv = cvy
                    
                    pred_cv = []
                    actual_cv = []
                    
                    pred_cv += [ s for (s, l) in zip(scores_cv, labels_cv) if s > 0 or l == 1]
                    actual_cv += [ l for (s, l) in zip(scores_cv, labels_cv) if s > 0 or l == 1]
                    
                    self.cvAPScore.append(average_precision_score(actual_cv, pred_cv))
    ...
    def predict(self, new_x, new_y):
        ...
        scores_test = newx.mm(self.w).data.numpy()
        
        self.topIndex = sorted(range(len(scores_test)), key=lambda i: scores_test[i])[-int(np.ceil(len(scores_test)/2)):]
        ...
        pred_test += [s[0] for (s, l) in zip(scores_test, labels_test) if s > 0 or l == 1]
        actual_test += [l for (s, l) in zip(scores_test, labels_test) if s > 0 or l == 1]
        
        self.testAPScore.append(average_precision_score(actual_test, pred_test))
...
def extract_features_cat(catIndex, labelForBboxAllImage, imageIdAndBbox, img_ids, feats, dataDir, dataType):
    ...
    for k in np.arange(positiveNum):
        ...
        idx1 = imageIdAndBbox[0].index(int(i))
    ...
    for m in np.arange(2 * positiveNum):
        ...
        featureLabels[k+m+1, 0:-1], featureLabels[k+m+1, -1] = bbox_feats2, -1
...
def classifier(dataDir, categoryId, lambd, learnrt, batchsz, loader,iternum=5):
    ...
    for i in np.arange(iternum):
        ...
        if i != iternum-1:
            lsvm.predict(features_train[np.where(img_cat_train == -1), :][0], img_cat_train[np.where(img_cat_train == -1)])
            ...
            features_train = np.vstack((features_train[np.where(img_cat_train == 1), :][0], np.vstack((features_train[np.where(img_cat_train == -1), :][0][lsvm.topIndex, :], features_train_new2))))
...
def calculate_test_ap(dataDir, coco, hardnum):
    ...
    # load features
    print("loading features...")
    [img_idsTest, featsTest] = cPickle.load(open(os.path.join(dataDir, 'features2_small', 'test2014.p'),'rb'), encoding='latin1')
    print("load features successfully...")
    ...
    while stepnum*stepsz < maxnum:
        ...
        print("predict "+str(stepnum*stepsz) + " out of " + str(maxnum))
    ...
    for i in np.arange(18):
        ...
        pred_test += [s for (s, l) in zip(wx[:, i], labelForBboxAllImageTest[:, i]) if s > threshold or l == 1]
        actual_test += [l for (s, l) in zip(wx[:, i], labelForBboxAllImageTest[:, i]) if s > threshold or l == 1]
        
        testaps[i] = average_precision_score(actual_test, pred_test)
...
dataDir = 'E:/U Washington/Study/STAT 548(Machine Learning for Big Data)/Data2'  # MAKE SURE THIS IS CORRECT!!!!
# dataType = 'val2014'
# dataType = 'test2014'
# dataType = 'train2014'
...
pool = ThreadPool(2)
results = pool.map(lambda catnum: classifier(dataDir, catnum, 5, 5e-6, 16, loader, 5), [i for i in [13, 16, 17, 2, 6, 8]])
pool.close()
pool.join()
