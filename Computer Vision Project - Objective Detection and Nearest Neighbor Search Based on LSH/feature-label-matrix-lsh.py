...
imageIdAndBbox = cPickle.load(open(os.path.join(dataDir, 'bboxes_retrieval', '{}_bboxes_retrieval.p'.format(dataType)), 'rb'), encoding='latin1')

imageId = np.unique(imageIdAndBbox[0]).tolist()
bbox = []
for i in np.arange(len(imageId)):
    idx = imageIdAndBbox[0].index(imageId[i])
    bbox.append(imageIdAndBbox[1][idx])

imageIdAndBbox = [imageId, bbox]
...
with open(os.path.join(dataDir, 'bboxes_retrieval', 'labelForBboxAllImage_{}.p'.format(dataType)), 'wb') as fp:
